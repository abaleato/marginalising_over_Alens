...
multiCorrectionInternal = numpy.nan_to_num(cK/clKK*clKK)
multiCorrectionCIB = numpy.nan_to_num(cI/clKK*clKI)
multiCorrectionKg = numpy.nan_to_num(cg/clKK*clKg)
...
for i in xrange(10):
    ...
    for j in xrange(len(lmatA)):
        ...
        b_intEr[j] = 0 #FIXMEEEEEEE
        b_intEr2[j] = 0 #FIXMEEEEEEE
#        print intEr
    intEr[lmatA>1500.] = 0.
    clBBADRand = numpy.dot(mat,phiBin/TCMB**2. * (1. - multiCorrectionInternal - multiCorrectionCIB*(1+2*intEr-b_intEr) - multiCorrectionKg*(1+2*intEr2-b_intEr2)))#clBBADRand = numpy.dot(mat,(1.-numpy.interp(lmatA,rho[0],rhoAlt**2.)*(1.+multiCorrection*intEr*2.+multiCorrectionKg*intEr2*2.))*phiBin/TCMB**2.)
